...
# Workshop material - Day #2 - MySQL Workbench/Cli/Curl/Browser to interact w/ application
  ...
  * Execute the ddl 
  * Click the table icon against the Room table in the right-hand side schemas section
  * Open database/database.dml in VS code
  * Copy the contents to MySQL Workbench
  * Execute the dml 
  * Click the table icon against the Room table in the right-hand side schemas section. You should see the data just inserted.
  * Play around with
    * SELECT with predicates
    * UPDATE statements with predicates
    * DELETE statements with predicates
    * At the end of all the playing around, leave the database with 5 records with roomNo 101 to 105
    
  * Next, we will build and run the java code
  * go to gitbash, ensure you are in workspace/MLPXX
  * `cd restservice/HotelManagement`
  * `mvn compile`
  * `mvn exec:java -Dexec.mainClass=com.hexaware.MLPXX.util.CliMain`
    * As expected, the cli displays only the roomNo for the guest; we need to do some code changes before the other attributes such as name will start appearing in the cli. But before that we will test the application as a REST service.
  ...
  * Build the war file as follows:
    ...
    * `cp target/MLPXX-0.0.1-SNAPSHOT.war /D/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16/webapps/MLPXX.war`
    * start tomcat and tail its logs
      * `cd D/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16`
      * `rm -rf logs/*`
      * `./bin/startup.sh`
      * `tail -f logs/*`
      * `curl -vvv http://localhost:8080/MLPXX/api/room | python -m json.tool`
      
   * `cd ../../webui/HMS-app/`
   * `npm install`
   * `ng build`
   * `cp ../../restservice/HotelManagement/target/MLPXX-0.0.1-SNAPSHOT.war ./MLPXX.war`
   * `cd dist`
   * `jar -uvf ../MLPXX.war *`
   * `cd ..`
   * `jar -tvf MLPXX.war`
   * `cp MLPXX.war /D/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16/webapps/MLPXX.war`
   * Notice that the tail terminal shows that the new version of the web application archive is now getting deployed
   * Open Chrome browser and navigate to http://localhost:8080/MLPXX/
