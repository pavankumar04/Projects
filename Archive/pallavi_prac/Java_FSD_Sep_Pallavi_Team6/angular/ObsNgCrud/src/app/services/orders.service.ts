...
export class OrdersService {
  ...
  listOrdersByOrderDate(date: Date): Observable<Orders[]> {
    return this.http.get<Orders[]>(`${baseUrl}/order/${date}`);
  }
  ...
  showAllCustomerOrdersByStatus(status: string, id: number): Observable<Orders[]>{
    return this.http.get<Orders[]>(`${baseUrl}/all/${status}/${id}`);
  }

  showAllOrdersByStatus(status: string): Observable<Orders[]>{
